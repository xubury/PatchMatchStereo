...
bool PatchMatchStereo::Init(int32_t width, int32_t height,
    ...
    // 灰度数据
    m_left_gray = CreateScope<uint8_t[]>(img_size);
    m_right_gray = CreateScope<uint8_t[]>(img_size);
    // 梯度数据
    m_left_grad = CreateScope<Gradient[]>(img_size);
    m_right_grad = CreateScope<Gradient[]>(img_size);
    // 代价数据
    m_left_cost = CreateScope<float[]>(img_size);
    m_right_cost = CreateScope<float[]>(img_size);
    // 视差图
    m_left_disparity = CreateScope<float[]>(img_size);
    m_right_disparity = CreateScope<float[]>(img_size);
    // 平面集
    m_left_plane = CreateScope<DisparityPlane[]>(img_size);
    m_right_plane = CreateScope<DisparityPlane[]>(img_size);

    m_is_initialized = m_left_grad && m_right_grad && m_left_disparity &&
                       m_right_disparity && m_left_cost && m_right_cost &&
                       m_left_plane && m_right_plane;
...
bool PatchMatchStereo::Match(const uint8_t* left_img, const uint8_t* right_img,
    ...
    if (left_disparity && m_left_disparity) {
        memcpy(left_disparity, m_left_disparity.get(),
...
void PatchMatchStereo::RandomInit() {
    ...
    for (int k = 0; k < 2; ++k) {
        auto disp_ptr =
            k == 0 ? m_left_disparity.get() : m_right_disparity.get();
        auto plane_ptr = k == 0 ? m_left_plane.get() : m_right_plane.get();
